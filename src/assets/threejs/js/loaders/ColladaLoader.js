...
ColladaLoader.prototype = {
  ...
  options: {
    set convertUpAxis(value) {
      console.warn(
        'ColladaLoader: options.convertUpAxis() has been removed. Up axis is converted automatically.'
      )
    }
  },
  ...
  parse: function(text, path) {
    ...
    function buildAnimationChannel(channel, inputSource, outputSource) {
      ...
      switch (transform) {
        ...
        case 'translate':
          console.warn(
            'ColladaLoader: Animation transform type "%s" not yet implemented.',
            transform
          )
          break
      ...
      for (let i = 0, l = xml.childNodes.length; i < l; i++) {
        ...
        switch (child.nodeName) {
          ...
          case 'morph':
            data.id = parseId(child.getAttribute('source'))
            console.warn(
              'ColladaLoader: Morph target animation not supported yet.'
            )
            break
        }
      }
    ...
    function buildMaterial(data) {
      ...
      function getTexture(textureObject) {
        ...
        if (sampler !== undefined) {
          ...
          const texture = textureLoader.load(getImage(surface.init_from))
            ...
            case 'VERTEX':
              for (const key in vertices) {
                ...
                switch (key) {
                  ...
                  default:
                    console.warn(
                      'ColladaLoader: Semantic "%s" not handled in geometry build process.',
                      key
                    )
                }
              }
    ...
    function buildGeometryData(primitive, source, offset, array) {
      ...
      if (primitive.vcount !== undefined) {
        ...
        if (maxcount > 0) {
          console.log(
            'ColladaLoader: Geometry has faces with more than 4 vertices.'
          )
        }
      } else {
      ...
      kinematics = {
        ...
        getJointValue: function(jointIndex) {
          ...
          if (jointData) {
            ...
            console.warn(
              'ColladaLoader: Joint ' + jointIndex + " doesn't exist."
            )
          }
        },
        ...
        setJointValue: function(jointIndex, value) {
          ...
          if (jointData) {
            ...
            if (value > joint.limits.max || value < joint.limits.min) {
              ...
              console.warn(
                'ColladaLoader: Joint ' + jointIndex + ' is static.'
              )
            } else {
              ...
              for (let i = 0; i < transforms.length; i++) {
                ...
                if (transform.sid && transform.sid.indexOf(jointIndex) !== -1) {
                  switch (joint.type) {
                    ...
                    default:
                      console.warn(
                        'ColladaLoader: Unknown joint type: ' + joint.type
                      )
                      break
                  }
                } else {
