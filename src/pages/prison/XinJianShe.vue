...
export default {
  ...
  methods: {
    ...
    fbxLoadA() {
      ...
      const uri = 'static/threejs/models/prison/scene/xinJianShe01.fbx'
      loader.load(uri, (result) => {
        console.log('result is ', result)
        const mesh = result.children[0]
        console.log('mesh is ', mesh)
        // correctly position the scene
        mesh.position.set(1580, 0, 1280)
        mesh.rotation.x = 0
    ...
    fbxLoadB() {
      const loader = new FBXLoader()
      const uri = 'static/threejs/models/prison/scene/xinJianShe02.fbx'
      loader.load(uri, (result) => {
        console.log('result is ', result)
        const mesh = result.children[0]
        console.log('mesh is ', mesh)
        mesh.position.set(770, 0, 1280)
        mesh.rotation.x = 0
        mesh.scale.set(0.06, 0.06, 0.06)
        // result.translateY(-13)
        // result.scene.rotateY(-0.3 * Math.PI)
