<template>
  <div class="example">
    <h2>Solar System——Use W A S D and your mouse to move</h2>
    <canvas id="main" :width="width" :height="height"/>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      lookAtMesh: null,
      Sun: null,
      mouse: new THREE.Vector2(), // 鼠标屏幕向量
      raycaster: new THREE.Raycaster() // 指向镭射
    }
  },
  created() {
  },
  ...
  methods: {
    ...
    displayPlanetName() {
      this.stars.forEach(star =>
        this.nameConstructor(star.name, this.stars.volume)
      )
      this.nameConstructor('Sun', 12)
    },
    /* 根据行星名字和体积构造显示名字*/
    nameConstructor(name, volume) {
      // name = new THREE.Font(name)
    ...
    move() {
      ...
      //     /* 面向相机*/
      //     displayName.lookAt(this.camera.position)
      //   }
      // } else {
