...
export default {
  ...
  methods: {
    ...
    initCamera() {
      ...
      this.camera.position.set(100, 0, 200)
    },
    initCameraControls() {
      // const cameraControls = new THREE.OrbitControls( this.camera, this.renderer.domElement );
    ...
    initModels() {
      ...
      const $d3g = {}
      this.d3threeD($d3g)
      this.addGeoObject(group, obj, $d3g)
    },

    addGeoObject(group, svgObject, $d3g) {
      const paths = svgObject.paths
      // const amounts = svgObject.amounts
      const depths = svgObject.amounts
      ...
      for (var i = 0; i < paths.length; i++) {
        ...
        for (var j = 0; j < simpleShapes.length; j++) {
          ...
          const shape3d = new THREE.ExtrudeBufferGeometry(simpleShape, {
            // amount: amount, THREE.ExtrudeBufferGeometry: amount has been renamed to depth.
            depth: depth,
            bevelEnabled: false
          })
