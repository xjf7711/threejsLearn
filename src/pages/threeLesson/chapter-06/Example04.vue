<template>
  <div class="example">
    <h2>Example 06.04 - Extrude TubeGeometry</h2>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tube: null,
      shape: null,
      step: 0, // call the render function
      spGroup: [], // the points group
      tubeMesh: null // the mesh
    }
  },
  mounted() {
  ...
  methods: {
    ...
    initGui() {
      this.guiControls = {
        ...
        newPoints: () => {
          const points = []
          for (let i = 0; i < this.guiControls.numberOfPoints; i++) {
          ...
          this.guiControls.points = points
          this.guiControls.redraw()
        },
        redraw: () => {
          this.scene.remove(this.spGroup)
          this.scene.remove(this.tubeMesh)
          this.generatePoints(this.guiControls.points, this.guiControls.segments, this.guiControls.radius, this.guiControls.radiusSegments, this.guiControls.closed)
        }
      }
      ...
      datGui.add(this.guiControls, 'newPoints')
      datGui.add(this.guiControls, 'numberOfPoints', 2, 15).step(1).onChange(this.guiControls.newPoints)
      datGui.add(this.guiControls, 'segments', 0, 200).step(1).onChange(this.guiControls.redraw)
      datGui.add(this.guiControls, 'radius', 0, 10).onChange(this.guiControls.redraw)
      datGui.add(this.guiControls, 'radiusSegments', 0, 100).step(1).onChange(this.guiControls.redraw)
      datGui.add(this.guiControls, 'closed').onChange(this.guiControls.redraw)

      this.guiControls.newPoints()
    },
    initStats() {
    ...
    animation() {
      ...
      this.tubeMesh.rotation.y = this.step += 0.01
    ...
    generatePoints(points, segments, radius, radiusSegments, closed) {
      ...
      this.spGroup = new THREE.Object3D()
      ...
      const tubeGeometry = new THREE.TubeGeometry(
        // new THREE.SplineCurve3(points), // THREE.SplineCurve3 has been deprecated. Use THREE.CatmullRomCurve3 instead.
    ...
    createMesh(geom) {
      ...
      const meshMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00, transparent: false, opacity: 0.7 })

      const wireFrameMat = new THREE.MeshBasicMaterial({ color: 0xffffff })
