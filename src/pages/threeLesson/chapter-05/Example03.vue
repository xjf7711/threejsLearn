<template>
  <div class="example">
    <h2>Example 05.03 - Basic 2D geometries - Shape</h2>
  </div>
</template>
  ...
  methods: {
    ...
    initGui() {
      this.guiControls = {
        ...
        asPoints: () => {
          ...
          this.scene.remove(this.shape)
          // create a new one
          this.shape = this.createLine(this.drawShape(), false)
          // add it to the scene.
          this.scene.add(this.shape)
        },
        asSpacedPoints: () => {
          ...
          this.scene.remove(this.shape)
          // create a new one
          this.shape = this.createLine(this.drawShape(), true)
          // add it to the scene.
          this.scene.add(this.shape)
        }
      }
      ...
      datGui.add(this.guiControls, 'asGeom')
      datGui.add(this.guiControls, 'asPoints')
      datGui.add(this.guiControls, 'asSpacedPoints')
    },
    initStats() {
    ...
    createLine(shape, spaced) {
      ...
      if (!spaced) {
        ...
        // Use new THREE.Geometry().setFromPoints( points ) instead.
        const spacedPoints = shape.getSpacedPoints(20) // 20无效
        console.log('extractPoints is ', spacedPoints)
        mesh = new THREE.Line(
          // shape.createSpacedPointsGeometry(20),
          new THREE.Geometry().setFromPoints(spacedPoints),
          new THREE.LineBasicMaterial({
            color: 0xff3333,
            linewidth: 2
          })
