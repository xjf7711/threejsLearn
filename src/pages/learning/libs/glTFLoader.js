  ...
  var ThreeGLTFLoader = Object.create(glTFParser, {
    ...
    threeJSMaterialType: {
      value: function(materialId, technique, values, params) {
        		...
        		if (technique && technique.description && technique.description.passes) { defaultPass = technique.description.passes.defaultPass }
        ...
        if (texture && textureParams) {
                	if (textureParams.wrapS == WebGLRenderingContext.REPEAT) { texture.wrapS = THREE.RepeatWrapping }

                	if (textureParams.wrapT == WebGLRenderingContext.REPEAT) { texture.wrapT = THREE.RepeatWrapping }

                	if (textureParams.magFilter == WebGLRenderingContext.LINEAR) { texture.magFilter = THREE.LinearFilter }
        ...
        if (texture && envMapParams) {
                	if (envMapParams.wrapS == WebGLRenderingContext.REPEAT) { texture.wrapS = THREE.RepeatWrapping }

                	if (envMapParams.wrapT == WebGLRenderingContext.REPEAT) { texture.wrapT = THREE.RepeatWrapping }

                	if (envMapParams.magFilter == WebGLRenderingContext.LINEAR) { texture.magFilter = THREE.LinearFilter }
        ...
        // hack hack hack
        if (texturePath && texturePath.toLowerCase().indexOf('.png') != -1) { params.transparent = true }
