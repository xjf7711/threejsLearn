...
const Physijs = (function() {
  ...
  const _Physijs = Physijs // used for noConflict method
  const Physijs = {} // object assigned to window.Physijs
  // let Eventable // class to provide simple event methods
  // let getObjectId // returns a unique ID for a Physijs mesh object
  // let getEulerXYZFromQuaternion
  // let getQuatertionFromEuler
  // let convertWorldPositionToObject // Converts a world-space position to object-space
  // let addObjectChildren

  let _temp1
  let _temp2
  const _temp_vector3_1 = new THREE.Vector3()
  const _temp_vector3_2 = new THREE.Vector3()
  const _temp_matrix4_1 = new THREE.Matrix4()
  const _quaternion_1 = new THREE.Quaternion()
  ...
  const convertWorldPositionToObject = function(position, object) {
  ...
  Physijs.createMaterial = function(material, friction, restitution) {
    var PhysijsMaterial = function() {}
    PhysijsMaterial.prototype = material
    PhysijsMaterial = new PhysijsMaterial()

    PhysijsMaterial._physijs = {
      id: material.id,
      friction: friction === undefined ? 0.8 : friction,
      restitution: restitution === undefined ? 0.2 : restitution
    }

    return PhysijsMaterial
  }
  ...
  Physijs.ConeTwistConstraint = function(objecta, objectb, position) {
    if (position === undefined) {
      console.log('Both objects must be defined in a ConeTwistConstraint.')
      return
    }
    this.type = 'conetwist'
    this.appliedImpulse = 0
